...
int main(){
    ...
    while(!q.empty()){
        ...
        for(int dir=0; dir<4; dir++){
            ...
            if(board[nx][ny]!=0) continue; //board가 0이 아니라면, -1(비어있거나), 1이상(익은 것, 혹은 방문한 것이므로)->즉 0일때만 방문가능
            day[nx][ny]=day[cur.x][cur.y]+1;
            board[nx][ny]=1; //방문한(익은) 토마토 체크.
            q.push({nx,ny}); 
            maxday=max(day[nx][ny],maxday); //걸리는 시간 업데이트
        }
    }
    for(int i=0; i<n; i++) //남아있는 토마토 중 익지않은 토마토가 있는지 체크
        for(int j=0; j<m; j++)
            if(board[i][j]==0){ //익지않은ㅣ 토마토가 있다면
                cout<<"-1";  //-1을 출력하고 종료
                return 0;
            }
